...
def cascade_fade_in(
    scene: Scene, group: Sequence[Mobject], lag_ratio: float = 0.1, direction: Any = DOWN * 0.2
) -> None:
...
def focus_zoom(
    scene: Scene, target: Mobject, zoom_factor: float = 0.5, duration: float = 1.5
) -> None:
    """Camera zoom into a specific target mobject. Requires scene to be a MovingCameraScene."""
    try:
        scene.play(
            scene.camera.frame.animate.set_width(target.width / zoom_factor).move_to(target),
            run_time=duration,
        )
...
def sweep_reveal(
    scene: Scene, mobject: Mobject, direction: Any = RIGHT, run_time: float = 1.0
) -> None:
...
def highlight_region(mobject: Mobject, opacity: float = 0.7) -> Mobject:
    ...
    hole = Rectangle(
        width=mobject.width + 0.6, height=mobject.height + 0.6, stroke_width=0, fill_opacity=0
    ).move_to(mobject)
