...
def test_api_to_video_flow_mocked(
    ...
    # 1. Create Project
    resp = api_client.post("/v1/projects", json={"title": "E2E Demo", "source_language": "vi"})
    assert resp.status_code == 201
    project_id = resp.json()["id"]

    # 2. Add Scene
    resp = api_client.post(f"/v1/projects/{project_id}/scenes", json={"scene_order": 0})
    assert resp.status_code == 201
