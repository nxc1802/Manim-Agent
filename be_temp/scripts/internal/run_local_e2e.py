...
def run_test_case(case, mode="auto"):
    ...
    project = check(
        requests.post(
            ...
            json={"title": f"E2E {test_id}", "description": prompt, "source_language": "vi"},
        ),
        201,
    )
    if not project:
    ...
    scene = check(
        requests.post(
            f"{API_BASE}/v1/projects/{project_id}/scenes",
            json={"scene_order": 0, "storyboard_text": prompt},
        ),
        201,
    )
    if not scene:
    ...
    check(
        requests.post(f"{API_BASE}/v1/scenes/{scene_id}/builder-review-loop", json={"mode": mode}),
        [200, 202],
    )
