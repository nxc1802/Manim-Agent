...
from ai_engine.builder_loop import (
    _agent_has_blocking,
    _code_review_passed,
    _get_convergence_timestamp,
    _run_agent_with_self_correction,
...
def test_code_review_passed_variants(base_cfg: BuilderReviewLoopConfig) -> None:
    ...
    assert (
        _code_review_passed(cfg=base_cfg, syntax_ok=False, policy_ok=True, agent_result=res)
        is False
    )
...
def test_get_convergence_timestamp() -> None:
    ...
    sync = {
        "version": "2",
        "granularity": "segment",
        "segments": [{"start": 0, "end": 10.5, "text": "test"}],
    }
...
async def test_run_agent_with_self_correction_success() -> None:
    ...
    # No schema
    res, v, m, s, u = await _run_agent_with_self_correction(
        "test", ok_call, None, result="plain text"
    )
    ...
    res, v, m, s, u = await _run_agent_with_self_correction(
        "test", ok_call, MySchema, result='{"foo": "bar"}'
    )
    assert res.foo == "bar"

    # With schema, result is dict
    res, v, m, s, u = await _run_agent_with_self_correction(
        "test", ok_call, MySchema, result={"foo": "baz"}
    )
    assert res.foo == "baz"
