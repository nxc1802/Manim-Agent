    ...
    try:
        async with websockets.connect(uri):
            ...
            async def trigger_event():
                ...
                try:
                    p = requests.post(
                        "http://localhost:8000/v1/projects", json={"title": "WS Test"}
                    ).json()
                    pid = p["id"]
                    s = requests.post(
                        f"http://localhost:8000/v1/projects/{pid}/scenes",
                        json={"scene_order": 0, "storyboard_text": "test"},
                    ).json()
                    sid = s["id"]
            ...
            async with websockets.connect(new_uri) as ws2:
                ...
                for _ in range(5):
                    ...
                    print(
                        f"Received event: {payload.get('component')} | {payload.get('phase')} | {payload.get('message')}"
                    )
