...
# Hugging Face Space Variables are available as Docker build arguments. These
# values are browser-public configuration; secrets must never use the VITE_
# prefix. Empty API/WS values intentionally select the same-origin defaults.
...
LABEL org.opencontainers.image.title="Manim Agent - Hugging Face single Space" \
      org.opencontainers.image.description="Trusted-input, single-tenant web/API/AI/render runtime"
...
# These values are public browser configuration. Preserve them in the runtime
# image so the production preflight can detect a misconfigured Space build.
ARG VITE_SUPABASE_URL=
ARG VITE_SUPABASE_PUBLISHABLE_KEY=
...
RUN groupadd --gid 1000 user \
    ...
    && mkdir -p /srv/backend /srv/ai_core /srv/shared /srv/frontend /srv/deploy \
...
COPY --from=frontend-build /build/frontend/dist /srv/frontend
...
RUN chmod 0755 /srv/deploy/entrypoint.sh /srv/deploy/service-entrypoint.sh
