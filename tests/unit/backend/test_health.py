...
def test_ready_ok(api_client: TestClient) -> None:
...
def test_project_not_found_stable_json(api_client: TestClient) -> None:
    pid = str(uuid.uuid4())
    response = api_client.get(f"/v1/projects/{pid}")
...
def test_invalid_uuid_path_returns_422_stable_json(api_client: TestClient) -> None:
    response = api_client.get("/v1/projects/not-a-uuid")
...
def test_correlation_id_roundtrip_header(api_client: TestClient) -> None:
    rid = "test-request-id-123"
    response = api_client.get("/health", headers={"X-Request-ID": rid})
    assert response.status_code == 200
