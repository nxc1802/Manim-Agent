...
from backend.api.deps import get_content_store, get_request_user_id, get_llm_client
...
def test_get_scene_success(mock_store, mock_user):
    ...
    mock_scene = Scene(
        id=sid, project_id=pid, scene_order=1,
...
async def test_generate_storyboard_success(mock_store, mock_user):
    ...
    mock_scene = Scene(
        id=sid, project_id=pid, scene_order=1,
        created_at=now, updated_at=now, storyboard_status="missing"
    )
    mock_store.get_scene.return_value = mock_scene
    mock_store.get_project.return_value = MagicMock(id=pid, user_id=mock_user, title="T", description="D", target_scenes=None)
