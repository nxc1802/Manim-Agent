...
def api_client(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> TestClient:
    ...
    def _fake_piper(_cfg: PiperRuntimeConfig, _text: str, out_wav: Path) -> list[dict[str, Any]]:
        blocks = [b.strip() for b in _text.split("\n\n") if b.strip()]
        _write_silent_wav(out_wav, 0.8 * len(blocks))
        return [{"text": b, "audio_duration": 0.8} for b in blocks]
